...
// Add the email search function
function initNetflixEmailSearch() {
    const searchForm = document.getElementById('netflix-search-form');
    if (!searchForm) return;

    const statusDiv = document.getElementById('netflix-search-status');
    const emailInput = document.getElementById('search-email');
    const passwordInput = document.getElementById('search-password');
    const submitBtn = document.getElementById('search-netflix-btn');
    const searchAgainBtn = document.getElementById('search-again-btn');
    const disconnectBtn = document.getElementById('disconnect-search-btn');
    ...
    searchForm.addEventListener('submit', function(e) {
        ...
        // Disable button and show loading status
        submitBtn.disabled = true;
        submitBtn.innerHTML = '<div class="loader" style="margin-right: 8px;"></div> Searching...';
        statusDiv.style.display = 'block';
        ...
        statusDiv.innerHTML = `
            <div class="status-indicator">
                ...
                <span class="status-text">Connecting to your email and searching for Netflix emails...</span>
            </div>
        `;

        fetch('/api/search-netflix-emails', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ email, password })
        })
        .then(res => res.json())
        .then(data => {
            if (data.success && data.emailInfo) {
                // Hide loading indicator and show results
                statusDiv.innerHTML = `
                    <div class="netflix-search-results" style="display: block;">
                        <div class="email-details-display">
                            <h4>Latest Netflix Email Found:</h4>
                            <div class="email-info">
                                <div class="info-item">
                                    <strong>Subject:</strong>
                                    <span id="email-subject">${escapeHTML(data.emailInfo.subject)}</span>
                                </div>
                                <div class="info-item">
                                    <strong>From:</strong>
                                    <span id="email-from">${escapeHTML(data.emailInfo.from)}</span>
                                </div>
                                <div class="info-item">
                                    <strong>Time:</strong>
                                    <span id="email-time">${escapeHTML(data.emailInfo.time)}</span>
                                </div>
                                ${data.emailInfo.otp ? `
                                <div class="info-item" style="background: rgba(229, 9, 20, 0.1); border-left-color: #22c55e;">
                                    <strong>OTP Code:</strong>
                                    <span id="email-otp" style="color: #22c55e; font-weight: bold; font-size: 1.2em; cursor: pointer;" 
                                          onclick="copyToClipboard('${data.emailInfo.otp}', 'OTP copied to clipboard!')" 
                                          title="Click to copy">📋 ${data.emailInfo.otp}</span>
                                </div>
                                ` : ''}
                            </div>
                        </div>
                        <button class="btn btn--primary" id="search-again-btn">Search Again</button>
                        <button class="btn btn--secondary" id="full-view-email-btn">📧 Full View Email</button>
                        <button class="btn btn--outline" id="disconnect-search-btn">Disconnect</button>
                    </div>
                `;
                
                // Add event listeners to the new buttons
                const newSearchAgainBtn = statusDiv.querySelector('#search-again-btn');
                const newFullViewBtn = statusDiv.querySelector('#full-view-email-btn');
                const newDisconnectBtn = statusDiv.querySelector('#disconnect-search-btn');
                
                newSearchAgainBtn.addEventListener('click', function() {
                    performNetflixSearch(email, password);
                });
                
                newFullViewBtn.addEventListener('click', function() {
                    viewFullEmail(email, password);
                });
                ...
                showNotification('Netflix email found successfully!', 'success');
                searchForm.style.display = 'none';
            } else {
                statusDiv.innerHTML = `<div class="error-message">❌ ${data.message || 'No Netflix emails found'}</div>`;
                showNotification(data.message || 'No Netflix emails found', 'info');
            }
        })
        .catch(err => {
            console.error('Netflix email search error:', err);
            const errorMessage = 'A server error occurred. Please try again later.';
            statusDiv.innerHTML = `<div class="error-message">❌ ${errorMessage}</div>`;
            showNotification(errorMessage, 'error');
        })
    ...
    function performNetflixSearch(email, password) {
        statusDiv.innerHTML = `
            <div class="status-indicator">
                <div class="loader"></div>
                <span class="status-text">Searching for new Netflix emails...</span>
            </div>
        `;
        ...
        .then(data => {
            if (data.success && data.emailInfo) {
                ...
                statusDiv.querySelector('#email-subject').textContent = data.emailInfo.subject;
                statusDiv.querySelector('#email-from').textContent = data.emailInfo.from;
                statusDiv.querySelector('#email-time').textContent = data.emailInfo.time;
                
                // Update or add OTP if available
                const existingOtpElement = statusDiv.querySelector('#email-otp');
                if (data.emailInfo.otp) {
                    if (existingOtpElement) {
                        existingOtpElement.textContent = `📋 ${data.emailInfo.otp}`;
                        existingOtpElement.onclick = () => copyToClipboard(data.emailInfo.otp, 'OTP copied to clipboard!');
                    } else {
                        // Add new OTP element
                        const emailInfo = statusDiv.querySelector('.email-info');
                        const otpElement = document.createElement('div');
                        otpElement.className = 'info-item';
                        otpElement.style.background = 'rgba(229, 9, 20, 0.1)';
                        otpElement.style.borderLeftColor = '#22c55e';
                        otpElement.innerHTML = `
                            ...
                            <span id="email-otp" style="color: #22c55e; font-weight: bold; font-size: 1.2em; cursor: pointer;" 
                                  ...
                                  title="Click to copy">📋 ${data.emailInfo.otp}</span>
                        `;
                        emailInfo.appendChild(otpElement);
                    }
                } else if (existingOtpElement) {
                    // Remove OTP element if no OTP found
                    existingOtpElement.parentElement.remove();
                }
                
                showNotification('Email search updated!', 'success');
            } else {
                showNotification(data.message || 'No new Netflix emails found', 'info');
            }
        })
        .catch(err => {
            console.error('Search again error:', err);
            showNotification('Error searching for emails', 'error');
        });
    }
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    initEmailVerification();
    initHouseholdUpdate();
    initNetflixEmailSearch(); // Add this line
});

// Function to view full email content in a modal
function viewFullEmail(email, password) {
    // Show loading modal
    showEmailModal('Loading email content...', true);
    
    fetch('/api/get-full-netflix-email', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ email, password })
    })
    ...
    .then(data => {
        if (data.success && data.emailContent) {
            ...
            // Create full email display HTML
            const fullEmailHTML = `
                <div class="full-email-details">
                    <div class="email-header-info">
                        <div class="email-field">
                            <strong>Subject:</strong>
                            <span>${escapeHTML(emailContent.subject)}</span>
                        </div>
                        <div class="email-field">
                            <strong>From:</strong>
                            <span>${escapeHTML(emailContent.from)}</span>
                        </div>
                        <div class="email-field">
                            <strong>To:</strong>
                            <span>${escapeHTML(emailContent.to)}</span>
                        </div>
                        <div class="email-field">
                            <strong>Date:</strong>
                            <span>${escapeHTML(emailContent.date)}</span>
                        </div>
                        ${emailContent.otp ? `
                        <div class="email-field" style="background: rgba(34, 197, 94, 0.1); border-left: 3px solid #22c55e; padding: 0.75rem; border-radius: 4px;">
                            <strong>🔑 OTP Code:</strong>
                            <span style="color: #22c55e; font-weight: bold; font-size: 1.3em; cursor: pointer;" 
                                  onclick="copyToClipboard('${emailContent.otp}', 'OTP copied to clipboard!')" 
                                  title="Click to copy">📋 ${emailContent.otp}</span>
                        </div>
                        ` : ''}
                    </div>
                    ...
                    <div class="email-body-content">
                        <h4>Email Content:</h4>
                        <div class="email-body-text">${escapeHTML(emailContent.bodyText || emailContent.bodyHtml || 'No content available')}</div>
                    </div>
                    
                    <div class="email-actions">
                        <button class="btn btn--outline" onclick="closeEmailModal()">Close</button>
                        ${emailContent.otp ? `<button class="btn btn--primary" onclick="copyToClipboard('${emailContent.otp}', 'OTP copied to clipboard!')">Copy OTP</button>` : ''}
                    </div>
                </div>
            `;
            
            showEmailModal(fullEmailHTML, false);
        } else {
            showEmailModal(`<div class="error-message">❌ ${data.message || 'Failed to load email content'}</div>`, false);
        }
    })
    .catch(err => {
        console.error('Full email fetch error:', err);
        showEmailModal('<div class="error-message">❌ Failed to load email content. Please try again.</div>', false);
    });
}
        ...
        <div class="email-modal-overlay" id="email-modal-overlay">
            <div class="email-modal">
                <div class="email-modal-header">
                    <h3>${isLoading ? 'Loading...' : 'Netflix Email Details'}</h3>
